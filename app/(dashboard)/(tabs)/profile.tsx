...
export default function ProfileScreen() {
  ...
  const recurringExpenses = userData.expenses.filter(expense => expense.isRecurring)
      ...
      {isLoading ? (
        ...
        <ScrollView>
          <View style={styles.headerContainer}>
            ...
            <View
              style={[styles.bubble, { top: 20, left: 20, width: 40, height: 40 }]}
            />
            <View
              style={[styles.bubble, { top: 60, right: 40, width: 30, height: 30 }]}
            />
            <View
              style={[
                styles.bubble,
          ...
          <View style={styles.section}>
            ...
            <View style={styles.financialCards}>
              <View style={[styles.card, styles.incomeCard]}>
                <FontAwesome6 name="arrow-trend-up" size={24} color={Colors.primary} />
                <Text style={styles.cardTitle}>Income</Text>
                <Text style={styles.cardAmount}>${totalIncome.toFixed(2)}</Text>
                <Text style={styles.cardSubtitle}>
                  {userData.income.length} source{userData.income.length !== 1 ? 's' : ''}
                </Text>
              </View>

              <View style={[styles.card, styles.expenseCard]}>
                <FontAwesome6 name="arrow-trend-down" size={24} color={Colors.error} />
                <Text style={styles.cardTitle}>Expenses</Text>
                <Text style={[styles.cardAmount, { color: Colors.error }]}>
                  ${totalExpenses.toFixed(2)}
                </Text>
                <Text style={styles.cardSubtitle}>
                  {userData.expenses.length} expense{userData.expenses.length !== 1 ? 's' : ''}
                </Text>
              </View>
            </View>

            <View style={[styles.card, styles.balanceCard]}>
              <AntDesign name="wallet" size={24} color={balance >= 0 ? Colors.success : Colors.error} />
              ...
              <View style={styles.balanceDetails}>
                ...
                <Text style={styles.balanceDetailText}>
                  Recurring Expenses: ${recurringExpenses.toFixed(2)}
                </Text>
              </View>
            </View>
          </View>
          ...
          <View style={styles.logoutContainer}>
            ...
            >
              <AntDesign name="logout" size={18} color={Colors.light} style={styles.logoutIcon} />
